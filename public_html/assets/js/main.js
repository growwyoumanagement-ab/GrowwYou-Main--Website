...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Mobile dropdowns (support multiple)
    const mobileDropdowns = document.querySelectorAll('.mobile-services-dropdown');
    mobileDropdowns.forEach(dropdown => {
        const trigger = dropdown.querySelector('.mobile-nav-trigger');
        if (trigger) {
            // Split trigger into link and arrow for separate actions
            const link = trigger.querySelector('.mobile-nav-text');
            const arrow = trigger.querySelector('.mobile-dropdown-icon');
            if (link) {
            ...
            // Make trigger accessible as a button-like control
            trigger.setAttribute('role', 'button');
            trigger.setAttribute('tabindex', '0');

            const toggleDropdown = (event) => {
                // Only toggle when the trigger itself (or its children) is activated
            ...
            trigger.addEventListener('keydown', toggleDropdown);
        }
    });

    // Make service category headings clickable (desktop & mobile)
    const categoryHeadings = document.querySelectorAll('.service-category > h4, .mobile-service-category > h4');
    categoryHeadings.forEach(h4 => {
        const slug = h4.textContent.trim().toLowerCase().replace(/&/g, '').replace(/\s+/g, '-');
